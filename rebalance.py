"""TQQQ/SGOV 1:1 再平衡演算法 — Phase 1：純邏輯，不碰網路。
...
策略規則：
  1. 永遠維持 TQQQ 總值 : SGOV 總值 = 1:1
  2. 比例超出 [1/1.05, 1.05]（約 [0.9524, 1.05]）就再平衡
  3. 任一腿是空的也算「需要再平衡」(初次建倉自動處理)
  4. 再平衡時，兩腿各自的目標 = (現金 + TQQQ 總值 + SGOV 總值) / 2
...
# 股票代號常數 — TQQQ 是 3 倍槓桿那斯達克 100 ETF；SGOV 是短期美國國債 ETF
TQQQ = "TQQQ"
SGOV = "SGOV"

# 再平衡觸發帶（band）
# UPPER_BAND: TQQQ/SGOV 比例 > 1.05 → 觸發，要砍 TQQQ
# LOWER_BAND: 比例 < 1/1.05 → 觸發，要砍 SGOV
# 為什麼用 1/1.05 而非直接 0.95：保持對稱（使用者規則是「超過 1:1.05」)
UPPER_BAND = Decimal("1.05")
LOWER_BAND = Decimal("1") / Decimal("1.05")  # ≈ 0.9524
...
class Decision:
    ...
    """
    action: str
    ratio: Decimal | None             # TQQQ總值/SGOV總值；任一腿為 0 時為 None
    tqqq_value: Decimal
    sgov_value: Decimal
    cash: Decimal
    target_per_leg: Decimal | None    # 再平衡時兩腿各自的目標金額
...
def decide(
    ...
    流程：
      1. 算出 TQQQ 和 SGOV 各自的「市值」(vt, vs)
      2. 判斷是否觸發再平衡（兩腿都 > 0 才看 ratio；任一腿是 0 直接觸發）
      3. 若不觸發 → 回傳 hold
      4. 若觸發 → 算每腿目標金額（含現金）→ 產生 sell/buy 計畫
    """
    ...
    # 用最近成交價算「現在的市值」；買賣的限價會在 live.py 另外算
    ...
    if vt + vs + cash == 0:
        return Decision("idle", None, vt, vs, cash, None, [], "Account is empty.")

    # 比例在帶內 → hold
    # 注意：即使帳上有現金，只要兩腿比例 1:1，按使用者規則就是不動
    # （想要「閒置現金自動部署」要改規則 — 目前嚴格照原指示）
    if not out_of_band:
        reason = f"ratio {ratio:.4f} within [{LOWER_BAND:.4f}, {UPPER_BAND}] — no action."
        if cash > 0:
            # 把「有閒置現金但不動」這件事明寫出來，方便日後檢視
            reason += f" (Note: ${cash:,.2f} cash sits idle — strategy says don't touch it.)"
        return Decision("hold", ratio, vt, vs, cash, None, [], reason)

    # ─── 進入再平衡分支 ───
    # 兩腿目標 = 全部帳戶價值的一半
    # 為什麼把 cash 算進去：再平衡這一刻，現金應該全部投入（達成 1:1）
    target = (vt + vs + cash) / 2
    orders: list[Order] = []

    # TQQQ：跟目標比，太多就賣、太少就買；剛好就不動
    if vt > target:
        sell_dollars = vt - target
        # sell_dollars / pt = 要賣多少股；先存抽象 quantity，整數化在 live.py
        orders.append(Order("sell", TQQQ, quantity=sell_dollars / pt))
    elif vt < target:
        orders.append(Order("buy", TQQQ, dollars=target - vt))

    # SGOV：同上邏輯，獨立判斷
    # 邏輯上 TQQQ 和 SGOV 不可能同時 sell（兩腿都 > target 代表總值算錯）
    # 但可能兩腿同時 buy（一腿剛好等於 target，另一腿低於），或一買一賣
    if vs > target:
        sell_dollars = vs - target
        orders.append(Order("sell", SGOV, quantity=sell_dollars / ps))
    elif vs < target:
        orders.append(Order("buy", SGOV, dollars=target - vs))

    if ratio is None:
        reason = "One or both legs empty — deploy cash to reach 1:1."
    else:
        reason = f"ratio {ratio:.4f} outside [{LOWER_BAND:.4f}, {UPPER_BAND}] — rebalance to 1:1."
    return Decision("rebalance", ratio, vt, vs, cash, target, orders, reason)
...
def render(d: Decision) -> str:
    ...
    lines = [
        ...
        f"Cash:           ${d.cash:,.2f}",
        f"Total:          ${d.tqqq_value + d.sgov_value + d.cash:,.2f}",
        f"Ratio (T/S):    {ratio_str}",
        f"Action:         {d.action}",
        f"Reason:         {d.reason}",
    ]
    if d.target_per_leg is not None:
        lines.append(f"Target per leg: ${d.target_per_leg:,.2f}")
    for o in d.orders:
        if o.side == "sell":
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--quotes", required=True, type=Path,
                    help="JSON from get_equity_quotes (必須含 TQQQ 和 SGOV)")
