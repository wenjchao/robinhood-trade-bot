...
├── local_runner.plist      ← launchd 範本（06:54 / 09:37 / 12:55 PDT 自動跑）
├── install.sh              ← 一鍵啟用本機備援
...
排程定義在 `.github/workflows/rebalance.yml`，預設每個交易日跑三個時段，每個時段排「主 + 備援」兩次：

| 時段 | 主 (ET) | 備援 (ET) | 用途 |
|---|---|---|---|
| 開盤掃 | 9:34 AM | 9:42 AM | 開盤後再平衡（避開開盤波動）|
| 盤中掃 | 12:17 PM | 12:25 PM | 盤中再平衡 |
| 收盤前掃 | 3:53 PM | 3:57 PM | 收盤前再平衡 |

為什麼排兩次：GitHub Actions cron 是 best-effort，**單次可能被延遲幾分鐘到幾小時**。bot 的「未成交單檢查」自動讓備援不會下重複單——主成功時備援會跳過整輪。
...
**前提**：Mac 在 PDT 06:54 / 09:37 / 12:55 時段開機（早上到中午本機時間）。launchd 用 **Mac 本機時區**——如果你的 Mac 不是 PDT/PST，要把 `local_runner.plist` 的 Hour 改成對應到 UTC 13:54 / 16:37 / 19:55 的當地時間。
...
之後就自動在 PDT 06:54 / 09:37 / 12:55 跑（= UTC 13:54 / 16:37 / 19:55 = ET 09:54 / 12:37 / 15:55）。Log 在 `/tmp/trade_bot_runner.log`。

排程設計：
- 06:54、09:37 PDT = GH backup +12 分鐘，純 backup 角色，避免開盤時段下單
- 12:55 PDT = 夾在 GH primary 跟 backup 中間，避免被市場 20:00 UTC 關盤截斷
- 冬令時間（11 月起，PST）：所有 Hour 欄位 -1，再跑 `bash install.sh`
